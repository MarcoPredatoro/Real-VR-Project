...
//         background = GameObject.Find("forest");
//         //background.GetComponentInChildren<AudioSource>().Play(backgroundMusic);
...
//         if (distance < threatRadius[0]) {
//             // Polo in the closest ring
//             Debug.Log("polo in the 3rd ring");
...
public class AttackRadius : MonoBehaviour
{
    ...
    void Update()
    {
        if (polo1 != null)
        {
            ...
            if (dis1 > threatRadius)
            {
                // Too far away, so should play the background sound
                SetMute(polo1, true);
                background.GetComponent<AudioSource>().mute = false;
            }
            else
            {
                // Close enough to hear the sound
                background.GetComponent<AudioSource>().mute = true;
                SetMute(polo1, false);
            }
        }
    ...
    void SetMute(GameObject polo, bool isMuted)
    {
        // Sets whether the child of the polo's audio sources are muted or not based on the bool
        polo.GetComponent<AudioSource>().mute = isMuted;
    }
